...
function parseChapters(description: string): Chapter[] {
  ...
  while ((match = chapterRegex.exec(description)) !== null) {
    ...
    chapters.push({
      time,
      seconds,
      title: emojiMatch ? title.replace(emojiMatch[0], '').trim() : title.trim(),
      emoji: emojiMatch?.[1],
    })
  }
...
function parseGuest(description: string): Guest | undefined {
  ...
  const linksSection = description.match(/Where to find[^:]*:\*\*<\/p>([\s\S]*?)(?:<p><strong>|$)/i)
  const links: { platform: string; url: string }[] = []
  
  if (linksSection) {
    const linkRegex = /(LinkedIn|Instagram|ResearchGate|Twitter|X):\s*<a[^>]*href="([^"]+)"/gi
    let linkMatch
...
function parseEpisodes(xmlText: string): Episode[] {
  ...
  for (const match of matches) {
    ...
    const enclosureMatch = itemContent.match(/<enclosure[^>]*url="([^"]*)"[^>]*(?:length="(\d+)")?/i)
    const audioUrl = enclosureMatch?.[1] || ""
    const audioSize = enclosureMatch?.[2] ? parseInt(enclosureMatch[2], 10) : undefined

    const duration = itemContent.match(/<itunes:duration>(.*?)<\/itunes:duration>/)?.[1]
    const imageUrl = itemContent.match(/<itunes:image[^>]*href="([^"]*)"/i)?.[1]
    ...
    // iTunes metadata
    const season = itemContent.match(/<itunes:season>(\d+)<\/itunes:season>/)?.[1]
    const episodeNum = itemContent.match(/<itunes:episode>(\d+)<\/itunes:episode>/)?.[1]
    const episodeType = itemContent.match(/<itunes:episodeType>(full|trailer|bonus)<\/itunes:episodeType>/)?.[1] as Episode['episodeType']
    const explicit = itemContent.includes('<itunes:explicit>true</itunes:explicit>') ||
                     itemContent.includes('<itunes:explicit>yes</itunes:explicit>')
    
    // Keywords from itunes:keywords
    const keywordsMatch = itemContent.match(/<itunes:keywords>(.*?)<\/itunes:keywords>/)
...
function parsePodcastMeta(xmlText: string): PodcastMeta {
  ...
  const description =
    channelContent.match(/<description><!\[CDATA\[([\s\S]*?)\]\]><\/description>/)?.[1] ||
    channelContent.match(/<description>([\s\S]*?)<\/description>/)?.[1] ||
    ""
    
  const author = channelContent.match(/<itunes:author>(.*?)<\/itunes:author>/)?.[1] || ""
  const email = channelContent.match(/<itunes:email>(.*?)<\/itunes:email>/)?.[1] || ""
  const copyright = channelContent.match(/<copyright>(.*?)<\/copyright>/)?.[1] || ""
  const language = channelContent.match(/<language>(.*?)<\/language>/)?.[1] || "en"
  
  // Category
  const categoryMatch = channelContent.match(/<itunes:category[^>]*text="([^"]*)"/)
  const category = categoryMatch?.[1] || ""
  const subcategoryMatch = channelContent.match(/<itunes:category[^>]*text="[^"]*"[^>]*>\s*<itunes:category[^>]*text="([^"]*)"/)
  const subcategory = subcategoryMatch?.[1] || ""
  
  const imageUrl = channelContent.match(/<itunes:image[^>]*href="([^"]*)"/i)?.[1] || ""
  const websiteUrl = channelContent.match(/<link>(.*?)<\/link>/)?.[1] || ""
  const feedUrl = channelContent.match(/<atom:link[^>]*href="([^"]*)"[^>]*rel="self"/i)?.[1] || RSS_FEED_URL
  const explicit = channelContent.includes('<itunes:explicit>true</itunes:explicit>') ||
...
// Cached RSS fetching using unstable_cache
const getCachedRSS = unstable_cache(
  async () => fetchRSS(),
  ['podcast-rss'],
  { revalidate: 300, tags: ['episodes'] } // 5 minutes
)

export async function getPodcastMeta(): Promise<PodcastMeta> {
  const xmlText = await getCachedRSS()
  return parsePodcastMeta(xmlText)
}

export async function getEpisodes(): Promise<Episode[]> {
  const xmlText = await getCachedRSS()
  return parseEpisodes(xmlText).slice(0, 20)
}
